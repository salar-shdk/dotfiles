...
def detect_cpu():
    ...
    lscpu   = run('lscpu', '').lower()
    if 'genuineintel' in lscpu or 'intel' in run('grep -m1 "vendor" /proc/cpuinfo', '').lower():
        vendor   = 'intel'
        temp_cmd = "sensors 2>/dev/null | awk '/^Package id 0:/{print $4; exit}'"
    else:
        vendor   = 'amd'
...
def detect_network():
    ...
    for line in out.splitlines():
        ...
        if 'dev' in parts:
            ...
            is_wifi = (os.path.isdir(f'/sys/class/net/{iface}/wireless') or
                       os.path.isdir(f'/sys/class/net/{iface}/phy80211'))
            return iface, is_wifi
...
def detect_battery():
    """Return battery name (e.g. 'BAT0') or None."""
    bats = sorted(glob.glob('/sys/class/power_supply/BAT*'))
    return os.path.basename(bats[0]) if bats else None


# ── Section builders (plain strings, no f-string on conky variables) ──────────
...
def build_cpu_cores(n_cores):
    ...
    for start in range(0, n_cores, cols):
        ...
        lines.append(''.join('${goto ' + str(col_w * (i + 1)) + '}${cpu cpu' + str(c) + '}%'
...
def build_gpu(gpu_type):
    if gpu_type == 'nvidia':
        return (
            ...
            '${color2}Memory:${color}${alignr}${execi 20 nvidia-smi --query-gpu=memory.used --format=csv,noheader}'
        ...
        if temp_path:
            temp_cmd = "awk '{printf \"%.0f°C\", $1/1000}' " + temp_path + " 2>/dev/null || echo N/A"
        else:
            temp_cmd = "sensors 2>/dev/null | awk '/^amdgpu/,0 {if(/edge:/) {print $2; exit}}'"
        return (
            '${color5}${font Roboto:size=10}A M D   G P U   ${hr 2}${font}${color}\n'
            '${color2}GPU:${color}${alignr}${execi 60 lspci | grep -iE "vga|3d" | grep -i amd | head -1 | sed "s/.*: //" | cut -c1-35}\n'
            '${color2}Temp:${color}${alignr}${execi 10 ' + temp_cmd + '}\n'
            '${color2}Utilization:${color}${alignr}${execi 5 cat /sys/class/drm/card0/device/gpu_busy_percent 2>/dev/null || echo N/A}%\n'
        ...
        return (
            '${color5}${font Roboto:size=10}I N T E L   G P U   ${hr 2}${font}${color}\n'
            '${color2}GPU:${color}${alignr}${execi 60 lspci | grep -iE "vga|display" | grep -i intel | head -1 | sed "s/.*: //" | cut -c1-35}\n'
            '${color2}Freq:${color}${alignr}${execi 5 cat /sys/class/drm/card0/gt_cur_freq_mhz 2>/dev/null || echo N/A} MHz\n'
            '${color2}Max Freq:${color}${alignr}${execi 60 cat /sys/class/drm/card0/gt_max_freq_mhz 2>/dev/null || echo N/A} MHz'
        )
...
def build_network(iface, is_wifi):
    ...
    common = (
        ...
        '${color2}External IP:${color}${alignr}${execi 600 curl -s https://api.ipify.org 2>/dev/null || echo "---"}\n'
        '${color2}Ping (8.8.8.8):${color}${alignr}'
        '${execi 10 ping -c 1 -W 1 8.8.8.8 2>/dev/null | grep "time=" | cut -d\'=\' -f4 | cut -d\' \' -f1 || echo "---"} ms\n'
        '${color2}Speed:${color} ${upspeedf ' + iface + '} kb/s ↑${alignr}${downspeedf ' + iface + '} kb/s ↓\n'
        '${color}${upspeedgraph ' + iface + ' 25,140 303030 4679cc}${alignr}${downspeedgraph ' + iface + ' 25,140 303030 d95468}\n'
        '${color2}Today:${color} ${totalup ' + iface + '} ↑${alignr}${totaldown ' + iface + '} ↓'
    )
    if is_wifi:
        wifi_lines = (
            '${color2}Status:${color}${alignr}${if_up ' + iface + '}${color7}Connected${else}${color9}Disconnected${endif}\n'
            '${color2}Signal:${color}${alignr}${wireless_link_qual_perc ' + iface + '}% '
        ...
        return header + wired_lines + common


def build_battery(bat):
    """Two lines for the SYSTEM section: percentage and power draw in watts."""
    if not bat:
        return ''
    power_path = f'/sys/class/power_supply/{bat}/power_now'
    if os.path.exists(power_path):
        power_cmd = "awk '{printf \"%.1f W\", $1/1000000}' /sys/class/power_supply/" + bat + "/power_now"
    else:
...
def main():
    ...
    # Battery lines go in SYSTEM section, with a blank line separator if present
